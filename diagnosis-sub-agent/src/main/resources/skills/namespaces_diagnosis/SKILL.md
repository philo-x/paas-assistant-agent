...
triggers:
  ...
  - "kubectl get pods 查不到资源且用户要求排查/定位原因，怀疑 Namespace 错误"
  - "跨 Namespace 访问 Service 失败（配合 networking_diagnosis 使用）"
  - "某个 Namespace 资源配额使用率接近上限，且用户要求分析或排查"
references:
...
```
工具：analyze(cluster, namespace=<ns>, filters=["Pod", "ReplicaSet", "StatefulSet"])
...
② 查看具体配额的使用情况
   ...
   工具：kubectl(cluster, args=["describe", "resourcequota", "<quota>", "-n", "<namespace>"])
...
① 跨 Namespace 查询 Pod（确认资源实际所在位置）
   ...
   工具：kubectl(cluster, args=["get", "pods", "-A", "-o", "wide"])
   → 找到实际运行的 Pod
...
> **Confidence:** High — `get_k8s_resource(cluster, kind=ResourceQuota)` 返回 `used=3.8/hard=4`；对 ReplicaSet 的 `list_k8s_event` 返回 `FailedCreate` 且 Message 包含 "forbidden: exceeded quota"。  
...
- 宏观排查全局 Namespace 列表及状态：
  `工具：kubectl(cluster, args=["get", "namespaces"])`
- 全局跨 Namespace 查看 Pod 信息：
  `工具：kubectl(cluster, args=["get", "pods", "-A", "-o", "wide"])`
- 查看资源配额（ResourceQuota）详细约束与实际额度：
  `工具：kubectl(cluster, args=["describe", "resourcequota", "<quota-name>", "-n", "<namespace>"])`
