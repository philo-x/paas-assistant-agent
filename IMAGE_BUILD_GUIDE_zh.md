...
本文档介绍如何构建独立的 PaaS Agent Assistant 项目及其各个模块。
...
位于 `paas-agent-assistant/build.sh`，用于批量构建多个子模块。
...
| | `platform-mcp-server` | 业务 MCP 服务器 |
| | `diagnosis-sub-agent` | 业务子 Agent |
| | `guide-sub-agent` | 咨询子 Agent |
...
包括：`supervisor-agent`（含前端）、`platform-mcp-server`、`diagnosis-sub-agent`、`guide-sub-agent`
...
> **注意**：`supervisor-agent` 的 Docker 镜像会通过多阶段构建把前端一起打包进去。如果你要生成本地可直接运行、且内含前端静态资源的 JAR，更适合使用 `./local-deploy.sh build`。
