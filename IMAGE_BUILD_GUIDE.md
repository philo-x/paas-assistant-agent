# PaaS Agent Assistant Build Guide

This document describes how to build the standalone PaaS Agent Assistant project and its modules.
...
cd paas-agent-assistant

# Build default modules (excluding infrastructure modules)
./build.sh -v 1.0.11 -p linux/amd64 -r registry.cn-hangzhou.aliyuncs.com/myapp --push

# Build all modules (including infrastructure modules)
./build.sh -m all -v 1.0.11 -p linux/amd64 -r registry.cn-hangzhou.aliyuncs.com/myapp --push
```
...
Located at `paas-agent-assistant/build.sh`, used for batch building multiple submodules.
...
| | `platform-mcp-server` | Business MCP server |
| | `diagnosis-sub-agent` | Business sub-agent |
| | `guide-sub-agent` | Consultation sub-agent |
...
Includes: `supervisor-agent` (with frontend), `platform-mcp-server`, `diagnosis-sub-agent`, `guide-sub-agent`
...
> **Note**: The `supervisor-agent` Docker image includes the frontend through a multi-stage build. If you need a local runnable JAR with bundled frontend assets, prefer `./local-deploy.sh build`.
