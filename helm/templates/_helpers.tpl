...
- name: MEM0_BASE_URL
  value: {{ .Values.mem0.baseUrl | quote }}
- name: MEM0_API_TYPE
  value: {{ .Values.mem0.apiType | quote }}
...
- name: K8SGPT_MCP_URL
  value: "http://k8sgpt-server:{{ .Values.k8sgpt.mcpPort }}/mcp"
- name: K8S_AUTH_MODE
  value: {{ .Values.k8s.authMode | quote }}
- name: KUBECONFIG_PATH
  value: {{ .Values.k8s.kubeconfigPath | quote }}
- name: K8S_CONTEXT
  value: {{ .Values.k8s.context | quote }}
- name: EXECUTION_APPROVAL_TTL_SECONDS
  value: {{ .Values.k8s.approvalTtlSeconds | quote }}
{{- end }}
